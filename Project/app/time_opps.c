...
uint64_t get_random_64bit_number(void){
	...
	volatile uint16_t seed_0_15 = TIM2_start(); //bit 16:0
	delay(100); // use counter time to generate random delay before next read
	volatile uint16_t seed_16_31 = TIM2_start(); //bit 31:15
	delay(1000);
	volatile uint16_t seed_32_47 = TIM2_start(); //bit 47:32
	delay(700);
	volatile uint16_t seed_48_63 = TIM2_start(); //bit 16:0
